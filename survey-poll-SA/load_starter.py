...
Drives a configurable rate of `record_response` standalone-activity executions
against the shared task queue `tq-survey-replay2026`. Both the local
survey-poll worker and the serverless Lambda variant poll this queue under the
same Worker Deployment version. With the local worker's
`max_concurrent_activities=1`, any parallel load immediately backs up the
activity task queue and provokes Lambda scale-ups.

Each respondent becomes one activity execution; answers are sampled from a
configurable Yes/No/Maybe distribution. Activity IDs embed a per-run prefix so
re-runs never collide. Fire-and-forget `start_activity` is bounded by a
...
DEFAULT_RETRY_POLICY = RetryPolicy(maximum_attempts=10)
...
async def _start_one(
    ...
    *,
    ...
    activity_id = f"survey-replay2026-{user_id}"
    ...
    try:
        ...
        logging.warning("start_activity failed for %s: %s", activity_id, e)
...
async def _drive_burst(
    ...
    *,
    ...
    logging.info(
        "Firing burst of %d activities (max_inflight=%d)", burst_size, max_inflight
    )
...
async def _await_results(
    ...
    async def wait_one(h: ActivityHandle) -> None:
        try:
            await h.result()
            stats.completed += 1
        except Exception:
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Load generator that drives standalone record_response activity "
            "executions to provoke serverless scale-ups alongside the local "
            "survey-poll worker."
        )
    )
    parser.add_argument(
        ...
        help=(
            ...
            "burst: fire N activities as fast as --max-inflight allows."
        ),
    )
    parser.add_argument(
        "--ramp-seconds",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="[ramp] Peak activity starts per second (default: 1.0).",
    )
    parser.add_argument(
        "--sustain-seconds",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="[burst] Number of activities to fire (default: 50).",
    )
    parser.add_argument(
        "--max-inflight",
        type=int,
        default=20,
        help="Max concurrent start_activity RPCs (default: 20).",
    )
    ...
    parser.add_argument(
        "--wait-for-results",
        action="store_true",
        help="Await every started activity after the load phase.",
    )
    parser.add_argument(
        "--result-timeout",
        type=float,
...
async def main() -> None:
    ...
    if args.wait_for_results and handles:
        logging.info(
            "Awaiting %d activity results (timeout=%.0fs)...",
            len(handles),
            args.result_timeout,
        )
