...
class TestCategoryAgent:
    ...
    async def test_categorize_office_supplies(self, sample_expense):
        ...
        with patch('openai_agents_expense.activities.web_search.web_search_activity') as mock_web_search:
            ...
            # Mock the agent's run method to return expected structure
            agent = CategoryAgent()
            ...
            with patch.object(agent, 'categorize', return_value=expected_result) as mock_categorize:
                result = await agent.categorize(sample_expense)
                
                assert result.category == "Office Supplies"
                assert result.confidence >= 0.9
                assert result.vendor_validation.is_legitimate is True
                mock_categorize.assert_called_once_with(sample_expense)

    @pytest.mark.asyncio
    async def test_categorize_suspicious_vendor(self, sample_expense):
        """Test CategoryAgent handles suspicious vendor"""
        from openai_agents_expense.agents.category_agent import CategoryAgent
        from openai_agents_expense.models import ExpenseCategory, VendorValidation
        ...
        with patch('openai_agents_expense.activities.web_search.web_search_activity') as mock_web_search:
            ...
            with patch.object(agent, 'categorize', return_value=expected_result):
                result = await agent.categorize(sample_expense)
...
class TestPolicyEvaluationAgent:
    ...
    async def test_evaluate_compliant_expense(self, sample_expense, sample_expense_category):
        ...
        from openai_agents_expense.agents.policy_evaluation_agent import PolicyEvaluationAgent
        ...
        with patch.object(agent, 'evaluate', return_value=expected_result) as mock_evaluate:
            ...
            mock_evaluate.assert_called_once_with(sample_expense, sample_expense_category)
...
class TestFraudAgent:
    ...
    async def test_assess_low_risk_expense(self, sample_expense, sample_expense_category):
        ...
        from openai_agents_expense.agents.fraud_agent import FraudAgent
...
class TestDecisionOrchestrationAgent:
    ...
    async def test_decide_auto_approve(self, sample_expense):
        ...
        from openai_agents_expense.agents.decision_orchestration_agent import DecisionOrchestrationAgent
...
class TestResponseAgent:
    ...
    async def test_generate_approval_response(self, sample_expense, sample_expense_category):
        ...
        from openai_agents_expense.agents.response_agent import ResponseAgent
