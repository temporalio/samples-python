...
async def notify_approver(request_info: dict) -> str:
    ...
    activity.logger.info(
        ...
        f"  To respond, run:\n"
        f"    python -m langgraph_plugin.graph_api.human_in_the_loop.approval_wait_condition.run_respond {workflow_id} --approve --reason 'Approved'\n"
        f"    python -m langgraph_plugin.graph_api.human_in_the_loop.approval_wait_condition.run_respond {workflow_id} --reject --reason 'Rejected'"
    )
    ...
    print("\nTo respond, run:")
    print(
        f"  Approve: uv run python -m langgraph_plugin.graph_api.human_in_the_loop.approval_wait_condition.run_respond {workflow_id} --approve --reason 'Your reason'"
    )
    print(
        f"  Reject:  uv run python -m langgraph_plugin.graph_api.human_in_the_loop.approval_wait_condition.run_respond {workflow_id} --reject --reason 'Your reason'"
    )
    print()
