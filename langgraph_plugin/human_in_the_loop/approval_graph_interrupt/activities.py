...
async def notify_approver(request_info: dict) -> str:
    ...
    activity.logger.info(
        ...
        f"    python -m langgraph_plugin.human_in_the_loop.approval_graph_interrupt.run_respond {workflow_id} --reject --reason 'Rejected'"
    )
    ...
    print("\nTo respond, run:")
    print(
        f"  Approve: uv run python -m langgraph_plugin.human_in_the_loop.approval_graph_interrupt.run_respond {workflow_id} --approve --reason 'Your reason'"
    )
    print(
        f"  Reject:  uv run python -m langgraph_plugin.human_in_the_loop.approval_graph_interrupt.run_respond {workflow_id} --reject --reason 'Your reason'"
    )
    print()
