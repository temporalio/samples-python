...
class TestExpenseWorkflow:
    ...
    async def test_workflow_approved_complete_flow(self, client: Client, env: WorkflowEnvironment):
    ...
    async def test_workflow_rejected_flow(self, client: Client, env: WorkflowEnvironment):
    ...
    async def test_workflow_other_decision_treated_as_rejected(self, client: Client, env: WorkflowEnvironment):
    ...
    async def test_workflow_create_expense_failure(self, client: Client, env: WorkflowEnvironment):
    ...
    async def test_workflow_wait_decision_failure(self, client: Client, env: WorkflowEnvironment):
    ...
    async def test_workflow_payment_failure(self, client: Client, env: WorkflowEnvironment):
    ...
    async def test_workflow_timeout_configuration(self, client: Client, env: WorkflowEnvironment):
        ...
        async with Worker(
            ...
            task_queue=task_queue,
            workflows=[SampleExpenseWorkflow],
            activities=[create_expense_timeout_check, wait_decision_timeout_check, payment_timeout_check],
        ):
            await client.execute_workflow(
                SampleExpenseWorkflow.run,
            ...
            create_timeout = next(call[1] for call in timeout_calls if call[0] == "create")
            wait_timeout = next(call[1] for call in timeout_calls if call[0] == "wait")
            payment_timeout = next(call[1] for call in timeout_calls if call[0] == "payment")
...
class TestExpenseActivities:
    ...
    async def test_wait_for_decision_activity_callback_registration_success(self, activity_env):
    ...
    async def test_wait_for_decision_activity_callback_registration_failure(self, activity_env):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            with pytest.raises(Exception, match="register callback failed status: ERROR:INVALID_ID"):
                await activity_env.run(wait_for_decision_activity, "test-expense-123")
    ...
    async def test_payment_activity_success(self, activity_env):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            ...
            mock_client_instance.get.assert_called_once_with(
                ...
                params={"is_api_call": "true", "type": "payment", "id": "test-expense-123"}
            )
...
class TestExpenseWorkflowWithMockServer:
    ...
    async def test_workflow_with_mock_server_approved(self, client: Client, env: WorkflowEnvironment):
    ...
    async def test_workflow_with_mock_server_rejected(self, client: Client, env: WorkflowEnvironment):
...
class TestExpenseWorkflowEdgeCases:
    ...
    async def test_workflow_with_retryable_activity_failures(self, client: Client, env: WorkflowEnvironment):
    ...
    async def test_workflow_logging_behavior(self, client: Client, env: WorkflowEnvironment):
    ...
    async def test_workflow_parameter_validation(self, client: Client, env: WorkflowEnvironment):
        ...
        async def create_expense_validate(expense_id: str) -> None:
            if not expense_id or expense_id.strip() == "":
                raise ApplicationError("expense id is empty or whitespace", non_retryable=True)
            return None
...
class TestExpenseWorkflowGoCompatibility:
    ...
    async def test_workflow_return_values_match_go(self, client: Client, env: WorkflowEnvironment):
        ...
        for decision, expected_result in test_cases:
            ...
            async with Worker(
                ...
                assert result == expected_result, f"Decision '{decision}' should return '{expected_result}', got '{result}'"

    async def test_activity_timeouts_match_go(self, client: Client, env: WorkflowEnvironment):
        ...
        async with Worker(
            ...
            task_queue=task_queue,
            workflows=[SampleExpenseWorkflow],
            activities=[create_with_timeout_check, wait_with_timeout_check, payment_with_timeout_check],
        ):
            await client.execute_workflow(
                SampleExpenseWorkflow.run,
                "test-expense-timeouts",
    ...
    async def test_error_handling_matches_go(self, client: Client, env: WorkflowEnvironment):
        ...
        for scenario_name, failing_activity in failure_scenarios:
            ...
            def create_failing_impl(activity_name):
                def failing_impl(expense_id):
                    raise ApplicationError(f"Activity {activity_name} failed", non_retryable=True)
                return failing_impl
            ...
            async with Worker(
                ...
                task_queue=task_queue,
                workflows=[SampleExpenseWorkflow],
                activities=[create_activity_impl, wait_activity_impl, payment_activity_impl],
            ):
