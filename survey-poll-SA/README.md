...
Each vote is a **standalone activity** invoked directly by the client — no
per-respondent workflow. The aggregator stays a workflow because it needs
durable in-memory state, signals, queries, and continue-as-new.

```
  starter.py / load_starter.py
          │ client.start_activity(record_response, …)
          ▼
  record_response (activity, on tq-survey-replay2026)
          ├─► boto3 S3 PutObject  (durable audit log)
          └─► signal PollAggregatorWorkflow.submit_vote
                (retried by the activity retry policy if it fails)
...
Why an activity instead of a workflow per respondent: the activity is one
retriable unit with at-least-once semantics. If the aggregator signal
fails, the whole activity retries — the S3 PUT redoes (idempotent on the
`user_id` key) and the signal retries. No silent loss like the previous
best-effort signal pattern. Cheaper in Temporal Cloud Actions, too, since
there is no workflow history per vote.
...
| `workflows.py` | `PollAggregatorWorkflow` (`submit_vote`, `reset`, `tally`, continue-as-new) |
| `activities.py` | `record_response` standalone activity — S3 PUT + signal aggregator + optional saturation loop |
...
- Deployment name: `survey-poll-SA` (the `SA` suffix marks the
  standalone-activity architecture; older runs used `survey-replay2026`
  with a per-respondent workflow)
- Build ID: `v1`
...
`load_starter.py` has two modes for many-vote runs.

**Burst mode** — fire N votes as fast as `--max-inflight` allows. Good for
"instant snapshot" demos, stress-testing the aggregator's signal path, and
reproducing the consistent-query buffer pressure scenario.
...
**Ramp mode** — linearly ramp from ~0/s to `--peak-rate` over
`--ramp-seconds`, then sustain at peak for `--sustain-seconds`. Good for
sustained load, demonstrating the auto-scaler's reactive *and* periodic
paths over a longer window, and modeling a real audience answering over
time. **This is the default mode** — running `load_starter.py` with no
`--mode` uses ramp.

```bash
# 2,500 people in an audience answering the poll over ~4.5 minutes
# (≈ 10 votes/s peak, 30s ramp-up, 4 min sustain ≈ 2,550 total)
TEMPORAL_CONFIG_FILE=temporal.toml mise exec -- uv run python load_starter.py \
    --mode ramp \
    --ramp-seconds 30 \
    --peak-rate 10 \
    --sustain-seconds 240 \
    --max-inflight 50 \
    --yes-pct 70 --maybe-pct 20 --no-pct 10
```

Approximate total votes is `peak_rate × sustain_seconds + peak_rate ×
ramp_seconds / 2`. Use that to size the run for any audience.

Each respondent is assigned a unique `user_id` (`user-<run_id>-<index>`).
Activity IDs are `survey-replay2026-<user_id>`.

**About dedup.** The default `ActivityIDConflictPolicy` (`FAIL`) rejects a
second `start_activity` with the same ID **only while the previous run is
still Running**. Once a vote activity Completes, the default
`ActivityIDReusePolicy` (`ALLOW_DUPLICATE`) lets the same `user_id` vote
...
`id_reuse_policy=ActivityIDReusePolicy.REJECT_DUPLICATE` on the
`start_activity` call. In scaling-demo mode (`SURVEY_DURATION_SECONDS=150`)
...
dashboard tally. Existing `record_response` activity executions are not
canceled.
...
The Lambda only registers the `record_response` activity. The aggregator
workflow is intentionally excluded — it's long-lived and belongs on a
durable host. Per-respondent workflows no longer exist; each vote is a
standalone activity, so an activity-only Lambda registration is enough.
...
### Break-glass: manual Lambda invocations

When the local worker is down or the auto-scaling controller is paused,
fire async Lambda invocations directly against the function ARN.
`lambda_invoker.py` mirrors the controller's call shape
(`InvocationType=Event`, no payload) — this bypasses Temporal entirely.

```bash
mise exec -- uv run python lambda_invoker.py \
  --function-arn arn:aws:lambda:us-west-2:123456789012:function:survey-replay2026-worker \
  --region us-west-2 \
  --count 5
```

Use `--dry-run` to verify CLI args without calling AWS, and `--max-inflight`
(default 10) to cap concurrent Invoke RPCs. Throttles are reported separately
in the summary so you can rerun with smaller bursts.
...
sends synthetic `submit_vote` signals, or seeds a fresh aggregator run
with a backfilled `TallyResult`.
